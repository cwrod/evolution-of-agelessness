...
if should_overwrite:
	with open("output/figures/data/MonteCarlo_"+str(dam_classes)+".csv","w") as f:
		header = []
...
for trial in range(trials):
	...
	for class_num in range(dam_classes):
		print(
			f"rmax{class_num + 1} = {rmax_range_vals[class_num][trial]}, beta{class_num + 1} = {beta_range_vals[class_num][trial]}, ",
			end="")
	...
	print(entry)
	with open("output/figures/data/MonteCarlo_" + str(dam_classes) + ".csv", "a") as f:
